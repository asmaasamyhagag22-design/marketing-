...
_OFFERINGS_GUIDANCE: dict[str, str] = {
    ...
    "restaurant": (
        "This is a RESTAURANT or CAFE. Offerings can be:\n"
        "  - food categories (e.g., 'grilled meats', 'desserts', 'salads')\n"
        "  - cuisine type (e.g., 'Egyptian cuisine', 'Eastern food', 'Italian')\n"
        "  - signature dishes when named on the page\n"
        "  - dining experience type ('family dining', 'fine dining', 'casual')\n"
        "  - service modes that the brand offers: delivery, takeaway, "
        "catering, dine-in — each only if a page/section mentions it\n"
        "  - multi-branch presence when locations are described\n"
        "If exact dish names aren't documented but the homepage clearly "
        "describes cuisine type or dining experience, capture those as "
        "broader offerings — do NOT return an empty list. Each must still "
        "cite a block_id with the relevant quote."
    ),
    "cafe": (
...
def build_offerings_prompt(
    ...
    return (
        ...
        + "- IMPORTANT: If the website clearly describes what the "
        "organization provides to its audience (even broadly — cuisine "
        "type, training track, product category), DO NOT return an empty "
        "list. Capture the broadest accurate offerings supported by "
