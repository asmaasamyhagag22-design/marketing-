...
def _system_prompt(n_scenes: int, language: str, mode: str = "generic",
    ...
    return (
        ...
        "- on_screen_text: keep the reel almost TEXT-FREE — the visuals and voice-over carry it. A "
        "caption is at most 4-5 words (never a sentence) and must stay on screen LONG ENOUGH TO READ. "
        "EXCEPT scene 0: its caption MUST name the brand or the featured product/service (a MUTED "
        ...
        "- duration_s: 2-4 — a FAST TikTok cut, BUT a scene that carries a CAPTION needs >=3s so it "
        "can be READ (owner: 'the text goes by too fast'); then a hard cut to the next.\n\n"
